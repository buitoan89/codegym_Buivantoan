...
insert into category(category_id,category_name) values(3,'Tin hoc');
...
insert into book(book_id, book_name, publisher, author, publish_year, publish_time, amount, image, category_id, `status`) values (4, 'Java', 'Codegym', 'Nguyên a', '2018-03-03',1,60000,'',3,1);
...
insert into borrow_order(student_number, book_id, start_borrow_date, end_borrow_date,status) values (1,1,'2019-12-04','2019-12-06',0);
insert into borrow_order(student_number, book_id, start_borrow_date, end_borrow_date,status) values (2,2,'2019-12-04','2019-12-06',0);
select student.student_name,book.book_name,borrow_order.status from borrow_order
inner join student on  student.student_number = borrow_order.student_number
inner join book on  book.book_id = borrow_order.book_id
where borrow_order.status = 0
